...
for(DGE in c(FALSE,TRUE)){
  for(cl in clusters){
    ...
    if (DGE) {
      ad$write_h5ad(paste0("BRIE2_USA/simulation_DGE/cell_info_",
                           cl, ".h5ad"))
      write.table(data.frame(index = colnames(sce), 
                             is_A = ifelse(sce$group == "A", 1, 0)),
                  file = paste0("BRIE2_USA/simulation_DGE/cell_info_", 
                                cl, ".tsv"),
                  quote = FALSE,
                  sep = "\t", row.names = FALSE)
    } else {
      ad$write_h5ad(paste0("BRIE2_USA/simulation/cell_info_",
                           cl, ".h5ad"))
      write.table(data.frame(index = colnames(sce), 
                             is_A = ifelse(sce$group == "A", 1, 0)),
                  file = paste0("BRIE2_USA/simulation/cell_info_", 
                                cl, ".tsv"),
                  quote = FALSE,
                  sep = "\t", row.names = FALSE)
    }

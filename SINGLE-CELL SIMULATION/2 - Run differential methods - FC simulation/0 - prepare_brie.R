...
load_abundances <- function (sample_id, cell_type, FC) {
  path <- ifelse(FC == 6, paste0("simulation_FC6/", paste0("normal", sample_id),
                             "_", cell_type), 
                 paste0("simulation_FC9/", paste0("normal", sample_id),
                        "_", cell_type))
...
for(FC in c(6,9)){
  for(cl in clusters){
    sces <- lapply(sample_ids, load_abundances, cell_type = cl, FC = FC)
    ...
    if (FC == 6) {
      ad$write_h5ad(paste0("BRIE2_USA/simulation_FC6/cell_info_",
                           cl, ".h5ad"))
      write.table(data.frame(index = colnames(sce), 
                             is_A = ifelse(sce$group == "A", 1, 0)),
                  file = paste0("BRIE2_USA/simulation_FC6/cell_info_", 
                                cl, ".tsv"),
                  quote = FALSE,
                  sep = "\t", row.names = FALSE)
    } else {
      ad$write_h5ad(paste0("BRIE2_USA/simulation_FC9/cell_info_",
                           cl, ".h5ad"))
      write.table(data.frame(index = colnames(sce), 
                             is_A = ifelse(sce$group == "A", 1, 0)),
                  file = paste0("BRIE2_USA/simulation_FC9/cell_info_", 
                                cl, ".tsv"),
                  quote = FALSE,
                  sep = "\t", row.names = FALSE)
    }

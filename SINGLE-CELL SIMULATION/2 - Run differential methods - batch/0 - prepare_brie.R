...
load_abundances <- function (sample_id, cell_type) {
  path <- paste0("simulation_batch/", paste0("normal", sample_id),
                 "_", cell_type)
...
for(cl in clusters){
  sces <- lapply(sample_ids, load_abundances, cell_type = cl)
  ...
  sce$batch <- ifelse(sce$sample_id %in% which(batch == "2"), "2", "1")
  ...
  ad = SCE2AnnData(sce, X_name = "counts")
  ad$write_h5ad(paste0("BRIE2_USA/simulation_batch/cell_info_",
                       cl, ".h5ad"))
  
  DF = data.frame(index = colnames(sce), 
                  is_A = ifelse(sce$group == "A", 1, 0),
                  is_batch = ifelse(sce$batch == "2", 1, 0))

  write.table(DF,
              file = paste0("BRIE2_USA/simulation_batch/cell_info_", 
                            cl, ".tsv"),
              quote = FALSE,
              sep = "\t", row.names = FALSE)
}

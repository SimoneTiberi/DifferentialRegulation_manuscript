...
if(FALSE){
  ...
  if(file.exists(file)){
    ...
    TIME_merged = rbind(TIME_merged, DEXSeq_TECs[1:4])
  ...
  if(file.exists(file)){
    ...
    TIME_merged = rbind(TIME_merged, eisaR[1:4])
    rm(eisaR); rm(TIME)
  }
  ...
  if(file.exists(file)){
    ...
    TIME_merged = rbind(TIME_merged, DRIMSeq[1:4])
    rm(DRIMSeq); rm(TIME)
  }
  ...
  if(file.exists(file)){
    ...
    TIME_merged = rbind(TIME_merged, DR[1:4])
    rm(DR); rm(TIME)
  }
  ...
  if(file.exists(file)){
    ...
    TIME_merged = rbind(TIME_merged, satuRn[1:4])
    rm(satuRn); rm(TIME)
  }
  ...
  if(file.exists(file)){
    ...
    TIME_merged = rbind(TIME_merged, SUPPA2[1:4])
    rm(SUPPA2); rm(TIME)
  }

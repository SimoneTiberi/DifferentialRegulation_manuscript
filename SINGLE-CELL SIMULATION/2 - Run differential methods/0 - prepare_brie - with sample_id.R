...
load_abundances <- function (sample_id, cell_type, DGE) {
  path <- ifelse(DGE, paste0("simulation_DGE/", paste0("normal", sample_id),
                             "_", cell_type), 
                 paste0("simulation/", paste0("normal", sample_id),
                        "_", cell_type))
...
for(DGE in c(FALSE,TRUE)){
  for(cl in clusters){
    sces <- lapply(sample_ids, load_abundances, cell_type = cl, DGE = DGE)
    ...
    DF = data.frame(index = colnames(sce), 
                    ...
                    is_2 = ifelse(sce$sample_id == 2, 1, 0),
                    is_3 = ifelse(sce$sample_id == 3, 1, 0),
                    is_4 = ifelse(sce$sample_id == 4, 1, 0))
    
    if (DGE) {
      ad$write_h5ad(paste0("BRIE2_USA_sample/simulation_DGE/cell_info_",
                           cl, ".h5ad"))
      write.table(DF,
                  file = paste0("BRIE2_USA_sample/simulation_DGE/cell_info_", 
                                cl, ".tsv"),
                  quote = FALSE,
                  sep = "\t", row.names = FALSE)
    } else {
      ad$write_h5ad(paste0("BRIE2_USA_sample/simulation/cell_info_",
                           cl, ".h5ad"))
      write.table(DF,
                  file = paste0("BRIE2_USA_sample/simulation/cell_info_", 
                                cl, ".tsv"),
                  quote = FALSE,
                  sep = "\t", row.names = FALSE)
    }

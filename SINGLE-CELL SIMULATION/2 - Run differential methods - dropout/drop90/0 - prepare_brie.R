...
load_abundances <- function (sample_id, cell_type) {
  path <- paste0("simulation_drop90/", paste0("normal", sample_id),
                 "_", cell_type)
...
for(cl in clusters){
  ...
  ad$write_h5ad(paste0("BRIE2_USA/simulation_drop90/cell_info_",
                       cl, ".h5ad"))
  write.table(data.frame(index = colnames(sce), 
                         is_A = ifelse(sce$group == "A", 1, 0)),
              file = paste0("BRIE2_USA/simulation_drop90/cell_info_", 
                            cl, ".tsv"),
              quote = FALSE,
              sep = "\t", row.names = FALSE)
}
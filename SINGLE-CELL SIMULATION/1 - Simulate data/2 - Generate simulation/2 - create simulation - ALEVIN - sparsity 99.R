...
sum(assays(sce)$TOT_counts); 
sum(assays(sce_final)$TOT_counts)

sum(assays(sce_final)$TOT_counts[sce_final$cell_type == CLUSTERS[1]])
sum(assays(sce_final)$TOT_counts[sce_final$cell_type == CLUSTERS[2]])
sum(assays(sce_final)$TOT_counts[sce_final$cell_type == CLUSTERS[3]])
...
non_zero_cells
saveRDS(sce_final, file = "03_data/mouse_simulation_data-ALEVIN-dropout99.rds")
